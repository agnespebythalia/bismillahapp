...
import com.bismillah.app.data.model.News
import com.bismillah.app.data.remote.NewsService
import com.bismillah.app.data.remote.RetrofitApi
...
class NewsRepository {
    ...
    suspend fun searchNews(query: String):ActionState<List<News>>{
        return try{
            val list= newsService.detailNews(query).await()
            ActionState(list.manga_list)
        }catch (e: Exception){
            ActionState(message = e.message,isSuccess = false)
        }
